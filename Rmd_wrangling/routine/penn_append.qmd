...
Keeping data in a central directory. Naming convention uses download date to distinguish batches. Will parse for redundancy below.
...
# can we move away from using column indices to trim? Using column names will throw an erro if the fulcrum metatada conventions ever change.
...
unique_visits2 <- bind_rows(survey_info, survey_acoustic, survey_capture, survey_edna, survey_ves) %>% 
