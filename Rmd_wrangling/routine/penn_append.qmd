...
fin_survey <- unique_survey %>% 
  ...
  group_by(date, site) %>% # not needed methink
  mutate(survey_time = if_else(is.na(survey_time), "night", survey_time)) %>% # again assumes all night. Instead implement time check
  select(!c(conductivity_us)) %>%
  ungroup() %>% # why this here, not above?
