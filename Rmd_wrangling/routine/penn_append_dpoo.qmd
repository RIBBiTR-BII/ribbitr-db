...
```{r}
# pending uploading of metadata

# pull relevant tables

# identify primary keys for each table
# identify natural keys for each table
...
# ?select only primary keys and natural keys from dependent tables?
# unless we are expecting to add to it... so maybe select everything.

db_survey <- tbl(connection, in_schema("survey_data", "survey"))
db_visit <- tbl(connection, in_schema("survey_data", "visit"))
db_site <- tbl(connection, in_schema("survey_data", "site"))
db_region <- tbl(connection, in_schema("survey_data", "region"))
db_location <- tbl(connection, in_schema("survey_data", "location"))
...
# use naming convention "raw_" to distinguish the source
