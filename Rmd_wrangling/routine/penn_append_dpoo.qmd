...
mdc = tbl(dbcon, "metadata_columns") %>%
  collect()

## Point to localdata directory
...
tbl_pkey = function(table_str, metadata_columns=mdc) {
  metadata_columns %>%
...
tbl_fkey = function(table_str, metadata_columns=mdc) {
  metadata_columns %>%
    filter(table_name == table_str,
           key_type == "FK") %>%
    pull(column_name)
...
tbl_nkey = function(table_str, metadata_columns=mdc) {
  metadata_columns %>%
    filter(table_name == table_str,
           natural_key) %>%
    pull(column_name)
...
tbl_keys = function(table_str, metadata_columns=mdc) {
  return(unique(c(
...
#               by = "site") %>%
#     left_join(db_region %>%
#                 select(tbl_keys("region", mdc)) %>%
#                 collect(),
#               by = "region_id") %>%
#     left_join(db_location %>%
#                 select( tbl_keys("location", mdc)) %>%
#                 collect(),
#               by = "location_id")
# }
...
clean_aural_obs <- raw_aural_obs %>%
  ...
  mutate(aural_id = fulcrum_id,
         ...
         comments_aural_ = str_replace_all(comments_aural, "\\n", ". ")) %>%
  select(-any_of(to_drop),
         -species_aural_other) %>%
  filter(!if_all(everything(), is.na)) # drop any rows all null
...
clean_capture_survey <- raw_capture_survey %>%
  ...
  mutate(site = str_to_lower(site),
         ...
         created_at = with_tz(ymd_hms(created_at, tz = "UTC"), "America/New_York"),
         survey_date = as.Date(ifelse(hour(created_at) > 12, date(created_at), date(created_at) - days(1)), origin = "1970-01-01"),
         date_date = as.Date(ifelse(survey_date - date > 1, date, survey_date), origin = "1970-01-01")) %>%
...
clean_capture_obs <- raw_capture_obs %>%
  ...
  select(-any_of(to_drop),
         -capture_status,
         -species_capture_other) %>% # capture status: was this individual successfully captured?
  filter(!if_all(everything(), is.na))  # drop any rows all null 
...
clean_sample_processing <- raw_sample_processing %>%
  ...
  mutate(site = str_to_lower(site),
         ...
         survey_date = as.Date(ifelse(hour(created_at) > 12, date(created_at), date(created_at) - days(1)), origin = "1970-01-01"),
         date = as.Date(ifelse(survey_date - date > 1, date, survey_date), origin = "1970-01-01")) %>%
  unite(observer_sample, c("processor", "processor_other"), sep=",", remove = TRUE, na.rm = TRUE) %>%
  select(-any_of(to_drop),
         survey_date) %>%
  filter(!if_all(everything(), is.na)) # drop any rows all null
...
# exploration
unicorn = unified_sample %>%
  anti_join(unified_capture, by=c("site", "date", "bag_id")) %>%
  # select(site, date, bag_id, species_capture, sample_id) %>%
  mutate(capture_id = NA) %>%
  filter(!is.na(bag_id))

monkey = unified_capture %>%
  anti_join(unified_sample, by=c("site", "date", "bag_id")) %>%
  # select(site, date, bag_id, species_capture, capture_id) %>%
  mutate(sample_id = NA) %>%
  filter(!is.na(bag_id))

unicorn_monkey = bind_rows(unicorn, monkey) %>%
  arrange(site, date, bag_id) %>%
  # group_by(site, date) %>%
  # mutate(temp_id = cur_group_id()) %>%
  # ungroup()
  select(site, date, bag_id, species_capture, capture_id, sample_id, start_time, end_time, observer_capture, observer_sample, comments_capture, comments_sample,)

write_csv(unicorn_monkey, here("staging", "unmatched_capture_sample.csv"))

ref = unified_capture %>%
  filter(is.na(sample_id) | is.na(capture_id),
         !is.na(bag_id)) %>%
  select(site, date, bag_id, species_capture, capture_id, sample_id) %>%
  arrange(site, date, bag_id) %>%
  group_by(site, date) %>%
  mutate(temp_id = cur_group_id()) %>%
  ungroup()
