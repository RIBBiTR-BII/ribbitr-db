...
clean_aural_obs <- raw_aural_obs %>%
  ...
  rename(species_aural = species_acoustic,
         ...
         aural_id = fulcrum_record_id) %>%
  mutate(species_aural = coalesce(species_aural, species_aural_other),
...
clean_ves_obs <- raw_ves_obs %>%
  clean_names() %>%
  rename(ves_id = fulcrum_record_id) %>%
  mutate(species_ves = coalesce(species_ves, species_ves_other),
...
clean_capture_obs <- raw_capture_obs %>%
  ...
  rename(body_temp_c = body_temperature,
         ...
         capture_id = fulcrum_record_id) %>%
  mutate(species_capture = coalesce(species_capture, species_capture_other),
...
gelled_aural = unified_aural %>%
  ...
  group_by_at(tbl_nkey("visit", metadata_columns)) %>%
  mutate(visit_id = ifelse(is.na(visit_id), UUIDgenerate(), visit_id)) %>%
  ungroup() %>%
  group_by_at(tbl_nkey("survey", metadata_columns)) %>%
