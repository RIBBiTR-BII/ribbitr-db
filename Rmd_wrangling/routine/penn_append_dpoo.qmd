...
```{r}
chain_aural = tbl_chain("capture", mdc, until="region")
chain_ves = tbl_chain("capture", mdc, until="region")
chain_capture = tbl_chain("capture", mdc, until="region")

db_aural_filter = tbl_join(dbcon, chain_aural) %>%
  filter(date >= dblog$date_min) %>%
  select(survey_id,
         visit_id) %>%
  collect()

db_capture_filter = tbl_join(dbcon, chain_capture) %>%
  filter(date >= dblog$date_min) %>%
  select(survey_id,
         visit_id) %>%
  collect()

db_ves_filter = tbl_join(dbcon, chain_ves) %>%
  filter(date >= dblog$date_min) %>%
  select(survey_id,
         visit_id) %>%
  collect()
...
tray = compare_for_staging(subset_aural_visit, db_visit %>% collect(), tbl_nkey('visit', mdc), return_all=TRUE)
comp_aural_visit$app

peace = tray$update %>%
  left_join(db_visit %>% collect(), by=tbl_nkey('visit', mdc))
...
#   filter(!is.na(bag_id))
# 
# sample_capture_unmatched = bind_rows(unicorn, monkey) %>%
