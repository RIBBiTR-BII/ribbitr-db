...
bd_unmapped = db_bd %>%
  anti_join(db_sample %>% 
              filter(sample_type == "bd"), by = c("bd_swab_id" = "sample_name")) %>%
  collect() %>%
  mutate(sample_id = map_chr(sample_id, ~ UUIDgenerate()))
...
# find bd_samples in bd results which don't have a corresponding observation
# generate sample id
# add sample id to sample lookup, with no capture id
