...
taxa_rp = replace_taxa("tadpole_species_1", NA, path = taxa_map_path)
taxa_rp = replace_taxa("tad_spp_2", NA, path = taxa_map_path)
...
sn_replace_region = sn_all %>%
  ...
  filter(taxa_trimmed != tolower(amphibiaweb_species) | taxa_trimmed != tolower(itis_canonical_current) | amphibiaweb_species != itis_canonical_current) %>%
  arrange(country)
...
sn_replace_pa = sn_replace_region %>%
  filter(country == "panama") %>%
  bind_rows(sn_all %>%
              filter(scientific_name == "uptidactylus_sarajay")) %>%
    group_by(scientific_name) %>%
