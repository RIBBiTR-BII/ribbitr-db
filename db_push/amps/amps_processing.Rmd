...
ri_wddir = here(ddir, "amps", "2026-03-09", "ribbitr")
...
list.files(path = here(ri_wddir, "gia_data"))
...
raw_peptide_sn_2025 = read_csv(here(ri_wddir, "sierra nevada", "SN_2025_peptide_quant.csv"))
raw_maldi_sn_2025 = read_csv(here(ri_wddir, "sierra nevada", "Sierra 2025 peak lists combined.csv"))
raw_intensity_sn_2025 = read_csv(here(ri_wddir, "sierra nevada", "Sierra Nevada 2025 MALDI amps intensity.csv"))
...
unified_intensity = bind_rows(raw_intensity_pa_2022_lwarz %>%
                              ...
                              raw_intensity_sn_2024 %>%
                                clean_names(),
                              raw_intensity_sn_2025 %>%
                                clean_names() %>%
                                # mutate(rana_2mbVariant_observed_peak = paste(str_extract_all(rana_2mb_variant_peak, "[0-9/]+"), sep = "/"),
                                #        rana_2mbVariant_observed_peak = if_else(rana_2mbVariant_observed_peak == "NA", NA, rana_2mbVariant_observed_peak),
                                #        rana_2mbVariant_intensity = NA) %>%
                                select(-rana_2mb_variant_peak)) %>%
  mutate(notes = ifelse(!is.na(amp_id_2),
                        gsub(", NA", "", paste0("Pooled fractions from: ",
                                                paste(amp_id,
                                                      amp_id_2,
...
clean_gia = bind_rows(raw_gia_pe %>%
  ...
  rename(sample_name = amp_id,
         ...
         notes = gia_notes) %>%
  mutate(amp_lab = "rs_lab",
         bd_isolate_id = recode_values(bd_isolate_id,
