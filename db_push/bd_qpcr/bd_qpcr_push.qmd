...
scrape_qpcr_xls <- function(raw_xls) {
  ...
  clean_xls = xls$working_results %>%
    clean_names() %>%
    mutate(q_pcr_plate = if_else("q_pcr_plate" %in% names(.), 
                                 .data[["q_pcr_plate"]], 
                                 rep(str_replace(xls$sample_setup[which(xls$sample_setup[,1] == "Experiment Name"),2], ".edt", ""), n())),
           extraction_plate = if_else("extraction_plate" %in% names(.), 
                                 extraction_plate,
                                 NA),
           plate_run = case_when(!str_detect(q_pcr_plate, "RERUN") ~ 1,
                                  str_detect(q_pcr_plate, "RERUN$") ~ 2,
                                  TRUE ~ as.numeric(str_extract(q_pcr_plate, "(?<=RERUN)\\d+")) + 1),
           ...
           master_mix = tolower(xls$sample_setup[which(xls$sample_setup[,1] == "Chemistry"),2]))
}
...
clean_pe_2022_02 = clean_pe_2022_02 %>%
...
clean_br_2022_01 = raw_br_2022_01$working_results %>%
  clean_names() %>%
