...
scrape_qpcr_xls <- function(raw_xls) {
  ...
  clean_xls <- xls$working_results %>%
    ...
    mutate(
      ...
      qpcr_date = parse_date_time(str_extract(qpcr_plate_name, "\\d{8}"), orders = "mdY"),
      extraction_date = parse_date_time(str_extract(extraction_plate_name, "\\d{8}"), orders = "mdY"),
...
gelled_results = clean_results %>%
  ...
  left_join(db_bd %>% 
              select(sample_name_bd,
                     result_id) %>%
              collect(), by = "sample_name_bd") %>%
  group_by(sample_name_bd,
           qpcr_plate_name,
           qpcr_well) %>%
  mutate(result_id = ifelse(is.na(result_id), UUIDgenerate(), result_id)) %>%
  ungroup()
...
peace = db_bd %>%
  collect() %>%
  mutate(src = "db") %>%
  inner_join(gelled_results %>%
               mutate(src = "csv"), by = "sample_name_bd")

issues = get_dupes(peace, sample_name_bd)

# great, no dupes between sources in inner join. inherit result_id by sample_name_bd
```
