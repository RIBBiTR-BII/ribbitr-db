...
clean_km = raw_km %>%
  ...
  mutate(plate_name = paste0(as.character(plate_id), "_km"),
         ...
         target = "ITS",
         replicate = 1,
         start_quant = if_else(is.na(start_quant), 0, start_quant),
         bd_load = if_else(is.na(bd_load), 0, bd_load)) %>%
...
clean_results = filtered_results %>%
  ...
  mutate(qpcr_dilution_factor = dilution * 10,
         ...
         bd_detected = case_match(target_quant,
                               0 ~ FALSE,
                               NA ~ NA,
                               .default = TRUE),
         replicate_detected = ifelse(bd_detected, "p", "n"),
         qpcr_plate_run = 1,
