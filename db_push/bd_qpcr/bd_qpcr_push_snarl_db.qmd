...
sn_results_ucsb = tbl(sncon, Id("results_snarl_import"))
sn_results_snarl = tbl(sncon, Id("results_ucsb_import"))
...
sn_results %>%
  inner_join(sn_results_ucsb, by = "sample_id") %>%
  count() %>%
  collect()

sn_results %>%
  inner_join(sn_results_snarl, by = "sample_id") %>%
  count() %>%
  collect()
...
clean_results = load_results %>%
  ...
  mutate(standard = coalesce(std_type, standard_type),
         ...
         detected = ifelse(!is.na(cycle_quant), TRUE, FALSE),
  ...
  group_by(sample_name_bd,
           qpcr_plate_name) %>%
  mutate(replicates_per_plate = max(replicate_id),
         average_ct = mean(average_ct, na.rm = TRUE),
         average_taget_quant = mean(average_target_quant, na.rm = TRUE)) %>%
  ungroup() %>%

unique(clean_results$standad)
unique(clean_results$target)


colnames(clean_results)

  rename(qpcr_plate_id = plate_id,
         qpcr_well = well_id)
...
clean_results = raw_bd_results %>%
  ...
  group_by(sample_name_bd,
           qpcr_plate_name) %>%
  mutate(replicates_per = max(replicate_id),
         average_ct = mean(average_ct, na.rm = TRUE),
         average_taget_quant = mean(average_target_quant, na.rm = TRUE)) %>%
  ungroup() %>%
