...
datetime_failed = ts_joined %>%
  filter(is.na(date_time_parsed))

tz_unique = unique(unlist(raw_time_zones))
  
# parse timezone
tzs = ts_joined %>%
  select(timezone) %>%
  distinct() %>%
  mutate(tz_clean = recode_values(timezone,
                               c("GMT-0:200", "GMT-02:00") ~ "Etc/GMT+2",
                               "GMT-04:00" ~ "Etc/GMT+4",
...
ts_clean = ts_clean_dates %>%
  ...
  mutate(site_microclimate = recode_values(site_hobo,
                           "Admin" ~ "admin_pond",
                           "Phelps" ~ "phelps_pond",
                           "RV" ~ "rv_pond",
                           "Tuttle" ~ "tuttle_pond",
                           ...
                           "SV05" ~ "s2",
                           default = site_hobo),
         height_cm = gsub("cm 2$", "cm", height_cm),
  ...
  mutate(units = recode_values(sensor_type,
...
# checks
height_fail = ts_clean %>%
  filter(is.na(height_cm))

check_1 = ts_clean_dates %>%
  filter(hobo_name == "BO_weatherstation")

check_2 = ts_clean %>%
  filter(hobo_name == "Vorisek_water_35cm 2")
...
ggsave(here("staging", "microclimate", "illuminance_local_time_brazil_02.png"), p_ill_br)
ggsave(here("staging", "microclimate", "illuminance_local_time_panama_02.png"), p_ill_pa)
ggsave(here("staging", "microclimate", "illuminance_local_time_california_02.png"), p_ill_ca)
ggsave(here("staging", "microclimate", "illuminance_local_time_pennsylvania_02.png"), p_ill_pe)
...
ill_raw = ts_joined %>%
  mutate(time = as_hms(date_time_parsed)) %>%
  select(date_time_parsed,
         date_time_raw,
         time,
         timezone,
...
p_ill_raw = ggplot(ill_raw %>%
                     filter(studyarea == "PA",
                            !(location %in% c("water", "soil"))), aes(x = time, y = intensity_lux, color = src)) +
  geom_point(alpha = 0.1) +
  facet_wrap(c("site", "timezone")) +
  theme_minimal() +
  scale_y_log10() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  scale_x_time(breaks = as_hms(c("04:00:00", "08:00:00", "12:00:00", "16:00:00", "20:00:00")),
...
p_ill_p_05 = ggplot(p_05 %>%
                     filter(studyarea == "PA",
                            !(location %in% c("water", "soil"))), aes(x = time, y = intensity_lux, color = src)) +
  geom_point(alpha = 0.1) +
  facet_wrap(c("location", "height")) +
  theme_minimal() +
  scale_y_log10() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  scale_x_time(breaks = as_hms(c("04:00:00", "08:00:00", "12:00:00", "16:00:00", "20:00:00")),
