...
sv_clean = raw_sv %>%
  mutate(sample_name_bd = paste0("pce_", id)) %>%
  select(sample_name_bd,
         Bd_presence,
         Bd_load_swab)

bd_data_sv = db_bd %>%
  filter(sample_name_bd %in% sv_clean$sample_name_bd) %>%
  collect()

bd_sv = bd_data_sv %>%
  full_join(sv_clean, by = "sample_name_bd") %>%
  mutate(quant_cycle = NA,
         target_quant = Bd_load_swab,
         bd_detected = target_quant > 0,
         extract_volume_uL = 5,
...
ls_clean = raw_ls %>%
  clean_names() %>%
  mutate(sample_name_bd = paste0("brls_", str_pad(sample_id, 3, pad = "0")))

bd_data_ls = db_bd %>%
  filter(sample_name_bd %in% ls_clean$sample_name_bd) %>%
  collect()
...
bd_ls = ls_clean %>%
  select(sample_name_bd,
         ...
         quantity_1,
         quantity_2) %>%
  left_join(bd_data_ls %>%
              select(sample_name_bd,
                     sample_id,
                     result_id), by = c("sample_name_bd")) %>%
  pivot_longer(cols = c("quantity_1", "quantity_2"),
               names_to = "reps",
               values_to = "quantity") %>%
  mutate(replicates = "duplicate",
         replicate_id = case_match(reps,
                                   "quantity_1" ~ 1,
                                   "quantity_2" ~ 2,
                                   .default = NA),
         result_id = if_else(replicate_id == 1, result_id, UUIDfromName("099634bb-4c89-4021-8fd6-f127ac6a1180", paste0("Bd", sample_name_bd, "_2"))),
         singular_sample_results = FALSE,
         quant_cycle = NA,
         target_quant = quantity,
         bd_detected = (quantity > 0),
         total_qpcr_volume_uL = NA,
         qpcr_dilution_factor = 1,
         template_dna_volume_uL = 4,
         extract_volume_uL = 4,
         swab_type = "mw113",
         ...
         qpcr_plate_name = paste0("BrLandscapes_", str_pad(plate, 3, pad = "0")),
         qpcr_well = as.character(well + replicate_id - 1),
         qpcr_date = NA,
