...
# visualize all translocations
```{r}
uni_id = get_dupes(revised_cmr, cmr_id) %>%
  select(cmr_id) %>%
  distinct() %>%
  pull(cmr_id)

unicorns = db_cmr %>%
  left_join(db_capture, by = "cmr_id") %>%
  left_join(db_survey, by = "survey_id") %>%
  left_join(db_visit, by = "visit_id") %>%
  left_join(db_site, by = "site_id") %>%
  left_join(db_region, by = "region_id") %>%
  filter(region == "california") %>%
  select(cmr_id, local_cmr_id, taxon_capture, date, site_id, site, site_latitude, site_longitude) %>%
  collect()

unicorn_ids = unicorns %>%
  group_by(local_cmr_id) %>%
  summarise(n = n_distinct(site)) %>%
  filter(n > 1)

true_unicorns = unicorns %>%
  filter(local_cmr_id %in% unicorn_ids$local_cmr_id) %>%
  group_by(local_cmr_id, site) %>%
  summarise(count = n()) %>%
  left_join(db_site %>%
              select(site,
                     site_latitude,
...
# write_csv(unicorns, here("staging", "sn_unicorns.csv"))

# Convert the point data to an sf object
points_sf <- true_unicorns %>%
  st_as_sf(coords = c("site_longitude", "site_latitude"), crs = 4326)

# Create lines between points with the same cmr_id
lines_sf <- points_sf %>%
  group_by(local_cmr_id) %>%
...
# Prepare points data
points_data <- points_sf %>%
  mutate(geometry_type = "point") %>%
  select(local_cmr_id, site, count, geometry_type)

# Prepare lines data
lines_data <- lines_sf %>%
  mutate(geometry_type = "line",
         site = NA_character_,
         count = NA_integer_) %>%
  select(local_cmr_id, site, count, geometry_type)

# Write points to shapefile
st_write(points_data, here("staging", "translocation_points_pit.shp"), delete_layer = TRUE)

# Write lines to shapefile
st_write(lines_data, here("staging", "translocation_lines_pit.shp"), delete_layer = TRUE)

# If you want to save the result as a shapefile
# st_write(result_sf, "unicorn_points_and_lines.shp")
...
ccs = db_capture %>%
  ...
  left_join(db_region, by = "region_id") %>%
  filter(region == "california") %>%
  select(taxon_capture, date, site_id, site, site_latitude, site_longitude) %>%
  collect()
...
points_data <- points_sf %>%
  mutate(geometry_type = "point") %>%
  select(site, count, geometry_type)

st_write(points_data, here("staging", "sn_captures.shp"), delete_layer = TRUE)
```