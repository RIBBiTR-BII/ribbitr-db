...
clean_data = bind_rows(clean_focal,
  ...
  mutate(transect_type = tolower(transect_type),
         ...
         taxon_capture = case_match(taxon_capture,
...
gelled_data = clean_data %>%
  ...
  mutate(start_time = as_hms(ifelse(start_time == Inf, NA, start_time)),
         ...
         capture_id_temp = map_chr(capture_id_temp, ~ ifelse(is.na(.x), UUIDgenerate(), .x)),
...
existing_data = db_sample %>%
  ...
  filter(sample_type == "bd") %>%
  select(any_of(c(tbl_pkey("sample", mdc), 
                tbl_nkey("sample", mdc),
