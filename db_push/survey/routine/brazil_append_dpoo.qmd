...
in_focal = import_list(here(wddir, '2025-01-27', "RIBBiTR_Brazil_FocalSp_2024.xlsx"), na = c("", "NA"))
raw_focal_2022 = in_focal[["2022"]]
raw_focal_2023 = in_focal[["2023"]]
raw_focal_2024 = in_focal[["2024"]]
raw_focal_2025 = import(here(wddir, "2026-01-27", "Ribbtr_2025_lks.xlsx"), na = c("", "NA"))

in_nonfocal = import_list(here(wddir, '2025-01-27', "RIBBiTR_Brazil_Non focal species_2024.xlsx"), na = c("", "NA"))
raw_nonfocal_2023 = in_nonfocal[["2023"]]
raw_nonfocal_2024 = in_nonfocal[["2024"]]

in_edna = import_list(here(wddir, '2025-01-27', "RIBBiTR_Brazil_eDNA_2024.xlsx"), na = c("", "NA"))
raw_edna_2022 = in_edna[["2022"]]
raw_edna_2023 = in_edna[["2023"]]
raw_edna_2024 = in_edna[["2024"]]

# # non RIBBiTR
raw_tissue = import_list(here(wddir, '2025-01-27', "Boraceia_species tissues_2024.xlsx"), na = c("", "NA"))[["Sheet1"]]
raw_loggers = read_csv(here(wddir, '2025-01-27', "RIBBiTR_Brazil_Dataloggers_2024_clean.csv"), na = c("", "NA"))
...
```{r}
focal_cols = compare_df_cols(raw_focal_2022,
                             raw_focal_2023,
                             raw_focal_2024,
                             raw_focal_2025)
...
clean_focal = bind_rows(raw_focal_2022 %>%
  ...
  mutate(time = as.hms(format(time, "%H:%M:%S")),
         date = ymd(date),
...
clean_capture = bind_rows(clean_focal,
  ...
  mutate(transect_type = tolower(transect_type),
         ...
         region = case_when(grepl("s\\d+", site) ~ "santa_virginia",
                            ...
                            TRUE ~ region),
         bad_lat_lon = (capture_latitude < -90 | capture_latitude > 90 | capture_longitude < -180 | capture_longitude > 180),
         capture_latitude = ifelse(bad_lat_lon, NA, capture_latitude),
         capture_longitude = ifelse(bad_lat_lon, NA, capture_longitude),
         ...
         observer_capture = ifelse(observer_capture == "", NA_character_, observer_capture),
...
clean_edna = bind_rows(raw_edna_2022 %>%
                       ...
                       raw_edna_2024 %>%
                         ...
                         mutate(control = as.logical(control),
  ...
  mutate(collection_time = as.hms(format(collection_time, "%H:%M:%S")),
         detection_type = "edna",
         ...
         edna_latitude = round(as.numeric(ifelse(edna_latitude == "NA", NA, edna_latitude)), digits = 5),
         edna_longitude = round(as.numeric(ifelse(edna_longitude == "NA", NA, edna_longitude)), digits = 5),
         ...
         time_of_day = case_when(collection_time >= as_hms("08:00:00") & collection_time < as_hms("20:00:00") ~ "day",
                                 ...
                                 .default = NA_character_),
         filter_method = tolower(filter_method),
         project_id = "ribbitr",
...
gelled_env = unified_env %>%
  ...
  mutate(uuid_name_env = paste0(survey_id, start_time, wind_speed_m_s, air_temp_c, water_temp_c, p_h, relative_humidity_percent),
         ...
         observers_survey = if_else(observers_survey == "", NA, observers_survey)) %>%
...
unified_edna = clean_edna %>%
  group_by(date) %>%
  mutate(
    # Find the first row index where both site and time_of_day are non-NA
    first_valid_row = which(!is.na(site) & !is.na(time_of_day))[1],
    possible_site = site[first_valid_row],
    possible_time_of_day = time_of_day[first_valid_row]
  ) %>%
  ungroup() %>%
  mutate(
    site = if_else(is.na(site) & negative_control, possible_site, site),
    time_of_day = if_else(is.na(time_of_day) & negative_control, possible_time_of_day, time_of_day)
  ) %>%
  select(-first_valid_row, -possible_site, -possible_time_of_day) %>%
  group_by(site,
           date,
           time_of_day,
           transect) %>%
  ...
  left_join(db_site %>%
            ...
            by = "site") %>%
  left_join(db_visit %>%
              select(visit_id, date, time_of_day, site_id) %>%
              collect(),
            by = c("date", "site_id", "time_of_day")) %>%
  left_join(db_survey %>%
              select(tbl_keys("survey", mdc)) %>%
              filter(!is.na(visit_id)) %>%
              collect(),
...
gelled_edna = unified_edna %>%
  ...
  mutate(start_timestamp_utc = start_timestamp_utc(date, start_time, local_tz),
         ...
         survey_id = ifelse(is.na(survey_id), 
                            ...
                            survey_id)) %>%
  ungroup() %>%
  mutate(uuid_name = paste0(survey_id, edna_sample_name, collection_time, filter_method, filter_start_time),
...
tryCatch(
  {
    db_taxa = db_rows_insert(dbcon, db_taxa, insert_capture_taxa, by="taxon_id")
    ...
    db_survey = db_rows_upsert(dbcon, db_survey, upsert_capture_survey, by="survey_id")
    # temp_capture_survey = stage_to_temp(dbcon, db_survey, upsert_capture_survey)
    # pointer = tbl(dbcon, temp_capture_survey)
    # rows_upsert(db_survey, pointer, by="survey_id", in_place=TRUE)
    
    db_capture = db_rows_upsert(dbcon, db_capture, upsert_capture, by="capture_id")
...
## unify
```{r}
unified_results = raw_focal_2025 %>%
  clean_names() %>%
  select(bd_swab_tube_id,
         date,
         bd_status,
         bd_load,
         bd_load_log) %>%
  filter(!is.na(bd_status)) %>%
  rename(sample_name = bd_swab_tube_id) %>%
  mutate(bd_detected = (bd_status == 1),
         replicates = "singlet",
         replicate_id = 1,
         cycle_quant = NA,
         target_quant = bd_load,
         ...
         its1_copies_per_standard_unit = 1,
         ...
         master_mix = "taqman",
         extraction_plate_name = NA,
         extraction_date = NA,
         extraction_kit = "qiagen_powersoil",
         extraction_lab = "becker_lab",
         qpcr_plate_name = "becker_ribbitr_2025",
         qpcr_well = as.character(row_number()),
         qpcr_date = NA,
         qpcr_machine = "quantstudio_3",
         qpcr_lab = "becker_lab",
         comments_qpcr = NA,
         singular_sample_results = TRUE,
         sample_type = "bd")

meow = unified_results %>%
  mutate(rat_1 = target_quant_per_swab / target_quant,
         rat_2 = bd_its1_copies_per_swab / target_quant)
...
gelled_results = unified_results %>%
  ...
  group_by(sample_name_bd,
           qpcr_plate_name,
           qpcr_well) %>%
  mutate(uuid_name = paste0(sample_name_bd, qpcr_well, qpcr_plate_name),
