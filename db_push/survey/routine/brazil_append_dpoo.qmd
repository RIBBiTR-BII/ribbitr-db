...
br_cap = db_capture %>%
  ...
  left_join(db_region, by = "region_id") %>%
  left_join(db_country, by = "country_id") %>%
  filter(visit_id %in% br_ves$visit_id) %>%
  collect()
...
clean_ls = raw_ls %>%
  ...
  mutate(date = ifelse(date == "27/09/2018", "9/27/2018", date),
         ...
         uuid_name_capture = paste0(project, date, sample_id),
         capture_id = UUIDfromName("e3c835a4-4c8d-491c-9970-011aba853686", uuid_name_capture))
...
clean_sv = raw_sv %>%
  ...
  mutate(date = mdy(date),
         ...
         capture_utm_zone = 23,
         project = "pce_santa_virginia",
         id = as.character(id),
         bd_swab_tube_id = paste0("ple_", id),
         sex = case_match(sex,
                          'M' ~ "male",
                          c("F","FO") ~ "female",
                          c("U", "J") ~ NA),
         ...
         capture_animal_state = case_match(dead,
                                           ...
                                           NA ~ NA,
                                           .default = "alive"),
         uuid_name_capture = paste0(project, date, id),
         capture_id = UUIDfromName("e3c835a4-4c8d-491c-9970-011aba853686", uuid_name_capture))
...
check = full_join(peace, tray$insert, by = c("date", "site_id"))

po = tray$orphan

pp = bind_rows(tray$orphan %>%
                 mutate(src = "db"),
               tray$insert %>%
                 mutate(src = "csv")) %>%
  left_join(db_site %>%
              collect(), by = "site_id") %>%
  arrange(date, site, src)
...
br_mys = db_sample %>%
  ...
  left_join(db_site, by = "site_id") %>%
  left_join(db_region, by = "region_id") %>%
  filter(visit_id %in% tray$orphan$visit_id) %>%
  collect()
...
### investigate related observations
```{r}
obs_capture = db_capture %>%
  left_join(db_survey, by = "survey_id") %>%
  left_join(db_visit, by = "visit_id") %>%
  left_join(db_site, by = "site_id") %>%
  left_join(db_region, by = "region_id") %>%
...
obs_capture_rel = obs_capture %>%
  filter(visit_id %in% vids) %>%
  select(region,
         site,
         date,
         time_of_day,
...
obs_capture_range = obs_capture %>%
  filter(region %in% dblog$region,
         date >= dblog$date_min,
         date <= dblog$date_max) %>%
  select(region,
         site,
