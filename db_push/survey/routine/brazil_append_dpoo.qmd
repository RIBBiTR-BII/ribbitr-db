...
peace = get_dupes(subset_env_survey, survey_id)
...
gelled_edna = unified_edna %>%
  ...
  mutate(start_timestamp_utc = start_timestamp_utc(date, start_time, local_tz),
         ...
         survey_id = ifelse(is.na(survey_id),
                            ...
                            survey_id)) %>%
  ungroup() %>%
  mutate(uuid_name = paste0(survey_id, edna_sample_name, edna_latitude, collection_time, collection_type),
...
tray = compare_for_staging(db_survey %>% collect(), subset_survey_capture, "survey_id", report = "survey_capture")

upsert_survey_capture = bind_rows(tray$insert,
                                  tray$update)
