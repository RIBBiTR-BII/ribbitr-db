...
clean_capture = raw_capture %>%
  ...
  mutate(detection_type = "capture",
         ...
         amp_id = ifelse(amps == 1, swab_id, NA),
         antibody_id = if_else(ampshcl == 1, swab_id, NA),
...
# value corrections
clean_survey_info$sample_location_utm_zone[clean_survey_info$temp_survey_id == "271fe5d3-ae5b-4c2e-bd1f-4d1f356dcc37"] = "17N"
...
clean_survey_info$sample_location_utm_zone[clean_survey_info$temp_survey_id == "6d327912-04e3-409d-8cd8-82cc3e202a84"] = "17N"
...
gelled_site = clean_visit_info %>%
  ...
  select(region, site, sample_location_utm_zone, sample_location_utme, sample_location_utmn, sample_location_elevation_m) %>% 
  group_by(site) %>% 
  slice(1) %>%
  left_join(db_site %>% collect(), by = c("site")) %>%
  mutate(utme = coalesce(utme, sample_location_utme),
         utmn = coalesce(utmn, sample_location_utmn),
         utm_zone = ifelse(is.na(utme), NA, coalesce(utm_zone, sample_location_utm_zone)),
         elevation_m = coalesce(elevation_m, sample_location_elevation_m)) %>%
  select(-region_id) %>%
...
env_cols = c("sample_location_description",
             "sample_location_utm_zone",
             "sample_location_utme",
             "sample_location_utmn",
             "sample_location_elevation_m",
             "pressureinhg",
