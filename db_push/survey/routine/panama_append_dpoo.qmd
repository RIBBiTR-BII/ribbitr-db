...
gelled_edna = unified_edna %>%
  ...
  ungroup() %>%
  group_by(date) %>%
  mutate(negative_control_edna_id = first(edna_id[negative_control]),
         negative_control_edna_id = ifelse(negative_control, NA, negative_control_edna_id)) %>%
  ungroup() %>%
  select(-fulcrum_id)
```
...
gelled_sample = unified_sample %>%
  ...
  mutate(negative_control_sample_id = first(sample_id[negative_control]),
         negative_control_sample_id = ifelse(negative_control, NA, negative_control_sample_id),
         capture_id = ifelse(negative_control, NA, capture_id)) %>%
  ungroup()

samps_no_nc = gelled_sample %>%
  filter(!negative_control,
         is.na(negative_control_sample_id),
