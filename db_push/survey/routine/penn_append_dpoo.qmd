...
gelled_env = unified_env %>%
  ...
  ungroup() %>%
  mutate(environmental_id = map_chr(environmental_id, ~ifelse(is.na(.x), UUIDgenerate(), .x)))
...
unified_capture = clean_capture_obs %>%
  ...
  mutate(taxon_capture = coalesce(species_capture, species_sample),
...
x_capture = (nrow(upsert_capture_survey) > 0) | (nrow(insert_capture_taxa) > 0) | (nrow(upsert_capture) > 0 | nrow(insert_cmr) > 0)
...
if (x_capture) {
  ...
  tryCatch(
    {
      temp_cmr = stage_to_temp(dbcon, db_cmr, insert_cmr)
      pointer = tbl(dbcon, temp_cmr)
      rows_insert(db_cmr, pointer, by="cmr_id", in_place=TRUE, conflict = "ignore")
