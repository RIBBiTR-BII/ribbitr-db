...
```{r}
librarian::shelf(tidyverse, dbplyr, here, janitor, lubridate, RPostgres, stringr, DBI, parsedate, uuid, hms, RIBBiTR-BII/ribbitrrr, readxl)
...
wddir_2022 = here(ddir, "pennsylvania", "fulcrum", "Fulcrum_Data_2022")
wddir = here(ddir, "pennsylvania", "fulcrum", '2025-10-23') # working data directory
...
raw_edna_2022 = read_xlsx(here(ddir, "pennsylvania", "edna", "eDNA Collection Data PENN 22-25_2025-10-20.xlsx"), sheet = "Pennsylvania '22")
raw_edna_2023 = read_xlsx(here(ddir, "pennsylvania", "edna", "eDNA Collection Data PENN 22-25_2025-10-20.xlsx"), sheet = "Pennsylvania '23")
raw_edna_2024 = read_xlsx(here(ddir, "pennsylvania", "edna", "eDNA Collection Data PENN 22-25_2025-10-20.xlsx"), sheet = "Pennsylvania '24")
raw_edna_2025 = read_xlsx(here(ddir, "pennsylvania", "edna", "eDNA Collection Data PENN 22-25_2025-10-20.xlsx"), sheet = "Pennsylvania '25")
...
clean_capture_obs = bind_rows(
  ...
  raw_capture_obs %>%
    rename(body_temp_c = body_temperature_c)) %>%
  clean_names() %>%
  rename(comments_capture = amphibian_comments,
...
clean_sample_obs = bind_rows(
  raw_sample_obs_2022_cs %>%
    ...
    mutate(bd_swab_id = gsub("BDSwab", "BdSwab", bd_swab_id),
           bd_swab_id_2 = microbiome_id,
           microbiome_swab_id_2 = if_else(grepl("BacSwab", bacterial_swab_id), bacterial_swab_id, NA)),
  raw_sample_obs) %>%
  clean_names() %>%
...
clean_edna = bind_rows(raw_edna_2022 %>%
                         select(-`Sample ID`),
                       raw_edna_2023 %>%
                         mutate(`mLs Filtered` = as.numeric(`mLs Filtered`)),
                       raw_edna_2024,
                       raw_edna_2025) %>%
  clean_names() %>%
  filter(nc_sample != "Sterile") %>%
  mutate(sample_id = if_else(is.na(sample_id),
                             gsub("_2\\d{1}_", "_", edited_sample_id),
                             sample_id),
         sample_id = gsub("eDNA_(\\d{3})$", "eDNA00\\1", sample_id),
         negative_control = nc_sample == "NC",
         filter_method = tolower(passive_active),
         site = gsub(" |-", "_", tolower(site)),
         site = if_else(site != "tryon_weber",
                        paste0(site, "_pond"),
                        site),
         detection_type = "edna",
         ...
         transect = NA,
         start_time = NA) %>%
  rename(edna_sample_name = edited_sample_id,
         filter_volume_ml = m_ls_filtered,
         date = collection_date,
         filter_date = filtration_date,
         comments_edna = filtering_notes) %>%
  left_join(clean_survey_info %>%
              select(site, date, time_of_day), by = c("site", "date"), relationship = "many-to-many") %>%
  arrange(date, time_of_day) %>%
  group_by(edna_sample_name) %>%
  slice_head(n = 1) %>%
  ungroup() %>%
  mutate(time_of_day = if_else(is.na(time_of_day), "night", time_of_day))
...
clean_sample_obs$bag_id[clean_sample_obs$fulcrum_id == "9f72d0b0-9252-46e6-8391-8531174e1f97"] = "BAG68"
clean_sample_obs$bag_id[clean_sample_obs$fulcrum_id == "a21f4b06-0219-425e-a3ee-e353aa3649c9"] = "BAG07"
...
clean_sample_obs$microbiome_swab_id_2[clean_sample_obs$fulcrum_id == "6095d594-6932-4d76-8171-8f241c24e2e4"] = "BacSwab00282"
...
clean_sample_obs$genetic_id[clean_sample_obs$fulcrum_id == "1f854c06-7073-4931-88b1-008a85ce739f"] = "Pe_Toe00000"
clean_sample_obs$genetic_id[clean_sample_obs$fulcrum_id == "f76279e2-987a-41eb-b6f0-b0e310342dbb"] = "Pe_Toe00029"
clean_sample_obs$bd_swab_id[clean_sample_obs$fulcrum_id == "eef67003-b1e6-4592-988d-bdd0851abba5"] = "Pe_Bd01307"
clean_sample_obs$bd_swab_id[clean_sample_obs$fulcrum_id == "b77060cc-2dcf-4667-bb3e-4e411460e290"] = "Pe_Bd01318"
clean_sample_obs$bd_swab_id[clean_sample_obs$fulcrum_id == "331bf6ec-0ac0-4ae3-931b-73caf48b86f5"] = "Pe_Bd01345"
clean_sample_obs$bd_swab_id[clean_sample_obs$fulcrum_id == "fde64a12-03e7-4feb-adc6-1f8b7dddc56e"] = "Pe_Bd01347"
...
# edna_visits = clean_edna %>%
#   select(date, site, time_of_day) %>%
#   distinct()
# 
# vids = edna_visits %>%
#   anti_join(unique_visit, by = c("date", "site", "time_of_day")) %>%
#   left_join(db_site %>%
...
gelled_edna = unified_edna %>%
  group_by_at(tbl_nkey("survey", mdc)) %>%
  mutate(uuid_name = paste0(visit_id, detection_type),
         survey_id = ifelse(is.na(survey_id),
                            UUIDfromName("5113bb6e-8cff-4a2d-9910-904403736cfb", uuid_name),
                            survey_id)) %>%
  ungroup() %>%
  mutate(uuid_name = paste0(edna_sample_name, site, date),
...
s1 = sample_lookup_gen(gelled_capture, "capture_id", c("bd_swab_id", "bd_swab_id_2"), "bd")
s2 = sample_lookup_gen(gelled_capture, "capture_id", c("genetic_id"), "genetic")
s3 = sample_lookup_gen(gelled_capture, "capture_id", c("microbiome_swab_id", "microbiome_swab_id_2"), "microbiome")
...
peace = compare_updates(tray, id_cols = c("capture_id"))
...
#   filter(capture_id %in% peas$capture_id_new) %>%
#   select(capture_id,
#          svl_mm,
#          body_mass_g,
#          date,
...
#   left_join(db_visit, by = "visit_id") %>%
#   left_join(db_site, by = "site_id") %>%
#   filter(capture_id %in% peas$capture_id_old) %>%
#   select(capture_id,
#          svl_mm,
#          body_mass_g,
#          date,
