...
# whats going on with the 2022 changesets?
```{r}
...
clean_edna_collection = raw_edna_collection %>%
  ...
  mutate(start_time = collection_time,
         ...
         filter_method = tolower(gsub("-", "_", filter_method))) %>%
  select(-any_of(to_drop))
...
gelled_sample = clean_sample %>%
  ...
  ungroup() %>%
  mutate(digits = str_extract(sample_name, "\\d{5}")) %>%
  filter(digits != "00000")
...
```{r}
upsert_sample %>%
  filter(!(capture_id %in% upsert_capture$capture_id))

d_new = upsert_capture %>%
  left_join(hope %>%
              select(capture_id_old,
                     capture_id_new) %>%
              distinct(), by = c("capture_id" = "capture_id_new")) %>%
  mutate(capture_id = capture_id_old) %>%
  arrange(capture_id)

d_old = db_capture %>%
  filter(capture_id %in% tray$update_old$capture_id) %>%
  collect() %>%
  arrange(capture_id)

common_cols = intersect(colnames(d_new), colnames(d_old))
...
cfs = compare_for_staging(d_old, d_new, "capture_id", report = "test")

meow = compare_updates(cfs)
woof = meow[[1]]


wtf = gelled_capture_clean %>%
  filter(capture_id %in% upsert_capture$capture_id)
```
