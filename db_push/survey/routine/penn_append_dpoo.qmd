...
gelled_env = unified_env %>%
  ...
  mutate(duration_minutes = calc_duration(start_time, end_time),
         survey_id = ifelse(is.na(survey_id), UUIDfromName("5113bb6e-8cff-4a2d-9910-904403736cfb", paste0(temp_survey_id, detection_type)), survey_id),) %>%
  ungroup() %>%
...
unified_edna = clean_edna_collection %>%
  full_join(clean_edna_filtering, by = c("fulcrum_id" = "fulcrum_parent_id")) %>%
  full_join(clean_edna_survey, by = c("fulcrum_parent_id" = "fulcrum_id")) %>%
...
gelled_edna = unified_edna %>%
  ...
  mutate(comments_survey = str_c(comments_survey, collapse = ", "),
         uuid_name = paste0(visit_id, fulcrum_id, latitude, longitude),
