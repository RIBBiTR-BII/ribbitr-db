...
# sites
```{r}
gelled_site = bind_rows(clean_capture %>%
                          select(sn_survey_id),
                        clean_ves %>%
                          select(sn_survey_id)) %>%
...
unified_capture = clean_capture %>%
  ...
  left_join(db_visit %>%
            ...
            by = c("site_id", "date", "time_of_day")) %>%
  left_join(db_survey %>%
              select(survey_id, visit_id, detection_type, transect, start_time, observers_survey) %>%
              filter(!is.na(visit_id)) %>%
              collect(),
            by = c("visit_id", "detection_type", "transect", "start_time", "observers_survey")) %>%
  filter(!is.na(site),
         !is.na(date)) %>%
