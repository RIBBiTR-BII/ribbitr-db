...
raw_edna_2023 = read_csv(here(wddir, "edna_sierra_2023_fullData_240130_lindauer.csv"))
...
gelled_edna = unified_edna %>%
  ...
  # try assigning negative control aligned with filter size and collection method
  group_by(visit_id, filter_method, filter_size_um) %>%
  ...
  ungroup() %>%
  # for those still without negative control, use, whatever is available
  group_by(visit_id, negative_control_edna_id) %>%
  mutate(negative_control_edna_id = if_else(is.na(negative_control_edna_id), first(edna_id[negative_control]), negative_control_edna_id),
         negative_control_edna_id = ifelse(negative_control, NA, negative_control_edna_id)) %>%
  ungroup()
```
