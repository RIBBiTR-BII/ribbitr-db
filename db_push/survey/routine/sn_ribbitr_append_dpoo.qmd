...
unified_capture = clean_capture %>%
  ...
  left_join(db_survey %>%
              select(survey_id, visit_id, detection_type, start_time) %>%
              filter(!is.na(visit_id)) %>%
              collect(),
            by = c("visit_id", "detection_type", "start_time")) %>%
  filter(!is.na(site),
         !is.na(date)) %>%
...
peace = compare_updates(tray, "capture_id")

p1 = peace[[15]]

r1 = subset_capture %>%
  filter(capture_id == "fa513e2c-ff90-4588-8a8a-1603395d5cbd")
r2 = db_capture %>%
  filter(capture_id == "fa513e2c-ff90-4588-8a8a-1603395d5cbd") %>%
  collect()
