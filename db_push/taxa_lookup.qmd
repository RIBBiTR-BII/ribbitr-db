...
clean_taxon_id = function(taxon_id) {
  return(tolower(gsub(" ", "_", taxon_id)))
}
...
revised_aural = db_aural %>%
  ...
  mutate(comments_aural = ifelse(taxon_aural %in% names(comment_taxa_dict), 
                                 ...
                                 comments_aural),
         taxon_aural = clean_taxon_id(taxon_aural),
...
revised_capture = db_capture %>%
  ...
  mutate(comments_capture = ifelse(taxon_capture %in% names(comment_taxa_dict), 
                                   ...
                                   comments_capture),
         taxon_capture = clean_taxon_id(taxon_capture),
...
revised_ves = db_ves %>%
  ...
  mutate(comments_ves = ifelse(taxon_ves %in% names(comment_taxa_dict), 
                               ...
                               comments_ves),
         taxon_ves = clean_taxon_id(taxon_ves),
...
revised_cmr = db_cmr %>%
  select(cmr_id,
         taxon_cmr) %>%
  collect() %>%
  mutate(taxon_cmr = clean_taxon_id(taxon_cmr),
         taxon_cmr = as.character(ifelse(taxon_cmr %in% names(clean_taxa_dict),
...
if (lookup_all) {
  ...
  taxa_revised_unique_new = c("foo",
                              setdiff(taxa_revised_unique, taxa_existing))
  
  taxa_revised_results = taxa_lookup_map(taxa_revised_unique_new) %>%
    filter(taxon_id != "foo")
}
