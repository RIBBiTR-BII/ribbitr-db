...
rb_singlet_dupes = rb_dupes %>%
  filter(replicates == "singlet",
         # qpcr_lab != "snarl",
         qpcr_date >= "2020-01-01")

rb_notsinglet_dupes = rb_dupes %>%
  filter(sample_name_bd %in% rb_singlet_dupes$sample_name_bd,
         replicates != "singlet")

# rids = rb_singlet_dupes$result_id
# 
# bd_drop = db_bd %>%
#   filter(result_id %in% rids)
# 
# db_bd = rows_delete(db_bd, bd_drop, by = "result_id", in_place = TRUE, unmatched = "ignore")
...
q1 = db_sample %>%
  ...
  left_join(db_visit, by = "visit_id") %>%
  left_join(db_site, by = "site_id") %>%
  filter(grepl("221120_J001", sample_name)  & sample_type == "bd") %>%
  select(sample_name,
         sample_type,
         taxon_capture,
         site,
...
q2 = db_sample %>%
  ...
  left_join(db_visit, by = "visit_id") %>%
  left_join(db_site, by = "site_id") %>%
  filter(grepl("230719_F021", sample_name)  & sample_type == "bd") %>%
  select(sample_name,
         sample_type,
         taxon_capture,
         site,
...
q3 = db_bd %>%
  ...
  left_join(db_visit, by = "visit_id") %>%
  left_join(db_site, by = "site_id") %>%
  filter(date == "2023-07-19" & sample_type == "bd") %>%
  select(result_id,
         sample_name_bd,
         sample_name,
         sample_type,
...
# samples with multiple results
q1_samp_dups = q1 %>%
  group_by(sample_id) %>%
  count() %>%
  left_join(q1, by = "sample_id") %>%
  filter(n > 1,
        !(n == 2 & replicates == "duplicate"),
        !(n == 3 & replicates == "triplicate"),
        !(n == 4 & replicates == "quadruplicate"),
        !(n == 5 & replicates == "quintuplicate")) %>%
  arrange(qpcr_plate_name,
          sample_name)

q1_plate_names = q1_samp_dups %>%
  group_by(qpcr_plate_name) %>%
  count()

q1_ribbitr_plates = q1_plate_names %>%
  filter(grepl("ribbitr", qpcr_plate_name, ignore.case = TRUE))

q1_drop = q1_samp_dups %>%
  filter(qpcr_plate_name %in% q1_ribbitr_plates$qpcr_plate_name)

sids = q1_drop %>%
  pull(sample_id)

# captures with multiple samples
q2_cap_dupe = q1 %>%
  select(capture_id,
         sample_id) %>%
  distinct() %>%
  ...
  count() %>%
  filter(n > 1) %>%
  left_join(q1, by = "capture_id") %>%
  arrange(capture_id, sample_name)

q2_plates = q2_cap_dupe %>%
  group_by(qpcr_plate_name) %>%
  count()

q2_plates_ribbitr = q2_plates %>%
  filter(grepl("ribbitr", qpcr_plate_name, ignore.case = TRUE))

q2_drop = q2_cap_dupe %>%
  filter(qpcr_plate_name %in% q2_plates_ribbitr$qpcr_plate_name) %>%
  select(capture_id) %>%
  distinct() %>%
  left_join(q2_cap_dupe, by = "capture_id") %>%
  filter(!is.na(capture_id))
