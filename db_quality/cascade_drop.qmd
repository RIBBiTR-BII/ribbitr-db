...
# associated capture samples with microbiome results
microbiome_samples = db_microbiome %>%
  select(sample_id) %>%
  distinct() %>%
  inner_join(drop_sample_all, by = "sample_id") %>%
  filter(!is.na(sample_id))
...
update_sample_microbiome = drop_sample_all %>%
  inner_join(microbiome_samples %>% select(sample_id), by = "sample_id") %>%
  collect()
...
drop_sample = drop_sample_all %>%
  anti_join(bd_samples, by = "sample_id") %>%
  anti_join(mucosome_samples, by = "sample_id") %>%
  anti_join(microbiome_samples, by = "sample_id") %>%
  anti_join(amp_samples, by = "sample_id")
...
## drop (actually no cascading here...)
```{r}
