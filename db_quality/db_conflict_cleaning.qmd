...
get_dupes(db_cmr %>% collect(), capture_mark_recapture)

t1 = db_cmr %>%
  distinct() %>%
  mutate(cmr_id = as.character(cmr_id)) %>%
  inner_join(db_capture, by="cmr_id") %>% collect()

t2 = db_cmr %>%
  distinct() %>%
  inner_join(db_capture, by = c("local_cmr_id" = "cmr_id")) %>% collect()
...
cmr_repeats = cmr_clean %>%
  group_by(local_cmr_id, site, species_capture) %>%
  mutate(cmr_count = n()) %>%
  ungroup() %>%
  filter(cmr_count > 1)
