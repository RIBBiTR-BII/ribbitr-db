...
# add hemisphere to utm zone
```{r}
rev_site = db_site %>%
  left_join(db_region, by = "region_id") %>%
  left_join(db_country, by = "country_id") %>%
  mutate(utm_zone = ifelse(country_name == "brazil", paste0(utm_zone, "S"), paste0(utm_zone, "N"))) %>%
...
db_site = db_site %>%
  rows_update(rev_site, by = "site_id", unmatched = "ignore", in_place = TRUE)
...
timestamp_func = function(df, reg_id) {
  tz = df %>%
  ...
  df_out = df %>%
    filter(region_id == reg_id) %>%
    mutate(start_timestamp = ymd_hms(paste(date, start_time), tz = tz),
           end_timestamp = ymd_hms(paste(date, end_time), tz = tz),
           ...
           end_timestamp = ymd_hms(paste(end_date, end_time), tz = tz),
           duration_minutes = as.integer(round(as.numeric(difftime(end_timestamp, start_timestamp, units = "mins")), digits = 0)))
}

region_id_list = survey_data %>%
  select(region_id) %>%
  distinct() %>%
  pull(region_id)

survey_calc = map(region_id_list, ~ timestamp_func(survey_data, .x))
...
# temp_survey = stage_to_temp(dbcon, db_survey, survey_calc_df)
# pointer = tbl(dbcon, temp_survey)
# rows_update(db_survey, pointer, by = "survey_id", in_place=TRUE, unmatched = "ignore")
