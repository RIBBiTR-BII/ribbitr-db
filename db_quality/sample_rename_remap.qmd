...
upsert_sample = bind_rows(sample_upsert_existant,
                          sample_upsert_nonexistant)

drop_sample = reconciled_samples %>%
  select(sample_id)
...
rename_remap = function(table_pointer, sample_col) {
...
update_amp_total = rename_remap(db_amp_total, "sample_name_amp")
update_amp_gia = rename_remap(db_amp_gia, "sample_name_amp")
update_amp_mp = rename_remap(db_amp_mp, "sample_name_amp")
update_amp_mi = rename_remap(db_amp_mi, "sample_name_amp")
update_bd = rename_remap(db_bd, "sample_name_bd")
update_muc_gia = rename_remap(db_muc_gia, "sample_name_mucosome")
...
tryCatch(
  {
    ...
    if (nrow(update_amp_total) > 0) {
      temp_amp_total = stage_to_temp(dbcon, db_amp_total, update_amp_total)
      pointer = tbl(dbcon, temp_amp_total)
      db_amp_total = db_amp_total %>%
        rows_update(pointer, by="result_id", in_place=TRUE, unmatched = "ignore")
    }
    
    if (nrow(update_amp_gia) > 0) {
      temp_amp_gia = stage_to_temp(dbcon, db_amp_gia, update_amp_gia)
      pointer = tbl(dbcon, temp_amp_gia)
      db_amp_gia = db_amp_gia %>%
        rows_update(pointer, by="result_id", in_place=TRUE, unmatched = "ignore")
    }
    
    if (nrow(update_amp_mp) > 0) {
      temp_amp_mp = stage_to_temp(dbcon, db_amp_mp, update_amp_mp)
      pointer = tbl(dbcon, temp_amp_mp)
      db_amp_mp = db_amp_mp %>%
        rows_update(pointer, by="result_id", in_place=TRUE, unmatched = "ignore")
    }
    
    if (nrow(update_amp_mi) > 0) {
      temp_amp_mi = stage_to_temp(dbcon, db_amp_mi, update_amp_mi)
      pointer = tbl(dbcon, temp_amp_mi)
      db_amp_mi = db_amp_mi %>%
        rows_update(pointer, by="result_id", in_place=TRUE, unmatched = "ignore")
    }
    ...
    if (nrow(update_muc_gia) > 0) {
      temp_muc_gia = stage_to_temp(dbcon, db_muc_gia, update_muc_gia)
      pointer = tbl(dbcon, temp_muc_gia)
      db_muc_gia = db_muc_gia %>%
        rows_update(pointer, by="result_id", in_place=TRUE, unmatched = "ignore")
    }
    
    temp_sample = stage_to_temp(dbcon, db_sample, drop_sample)
    pointer = tbl(dbcon, temp_sample)
    db_sample = db_sample %>%
      rows_delete(pointer, by="sample_id", in_place=TRUE, unmatched = "ignore")
