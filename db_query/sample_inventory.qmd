...
region_last_capture_date = db_capture %>%
  left_join(db_survey, by = "survey_id") %>%
  left_join(db_visit, by = "visit_id") %>%
  left_join(db_site, by = "site_id") %>%
  left_join(db_region, by = "region_id") %>%
  ...
  summarise(latest_capture = max(date, na.rm = TRUE),
            .groups = "drop") %>%
  collect()
...
# amp
amp_results_unmatched = amp_tables %>%
  filter(results & !survey)
...
data_muc = db_mucosome %>%
  ...
  left_join(db_region, by = "region_id") %>%
  left_join(db_country, by = "country_id") %>%
  select(all_of(colnames(db_mucosome)),
         capture_id,
         taxon_capture,
         date,
         site,
