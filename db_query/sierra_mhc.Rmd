...
## spatial scope: aggregate to the population level
...
pop_oi = site_oi %>%
  select(population, name_ima, site) %>%
  group_by(population) %>%
  summarise(name_ima = first(name_ima),
...
Identify visits to sites of interest, grouping by population and survey period, defined as:
...
population_check = survey_select %>%
  filter(site %in% c("72336",
                     "72808",
                     "11008",
                     "11009")) %>%
  select(population,
         ...
         detection_type,
         survey_period) %>%
  arrange(population, date, site, detection_type)
...
data_out = abundance_stats %>%
  full_join(bd_stats, by = c("population", "year")) %>%
  arrange(population,
          year) %>%
  left_join(pop_oi, by = "population") %>%
  select(population,
         name_ima,
         sites,
         everything())
