...
librarian::shelf(tidyverse, DBI, RPostgres, here, janitor, RIBBiTR-BII/ribbitrrr)
...
## Patch for simultanious primary and foreign key status
```{r}
# 
# consolidate_keys <- function(df) {
#   df %>%
#     group_by(table_schema, table_name, column_name) %>%
#     summarise(across(everything(), function(col) {
#       if (n_distinct(col) == 1) {
#         first(col)
#       } else if ("PK" %in% key_type && "FK" %in% key_type) {
#         if (cur_column() == "key_type") {
#           "PF"
#         } else {
#           na.omit(col)[1]
#         }
...
# 
# for (schema in schemas) {
#   pg_column_data[[schema]] = consolidate_keys(pg_column_data[[schema]])
# }
```
...
## drop orphans
```{r}
# does not work yet, tables don't share same source...
...
tryCatch(
  {
    for (schema in schemas) {
      ...
      if (nrow(meta_column_upsert[[schema]]) > 0) {
        # drop orphans
        rows_upsert(meta_column_data[[schema]], meta_column_orphan[[schema]], by=columns_pkey, in_place=TRUE)
